...
impl PieceConfig {
    pub fn new(piece_type: PieceType, blocks:[Block; 4]) -> Self {
        let color: Color = Color::Srgba(match piece_type {
...
pub fn auto_generate_new_piece(
    ...
    if query.is_empty() {
        let piece_config: PieceConfig = piece_queue.0.pop_front().unwrap();
        // 生成新的四格骨牌
        let color: Color = piece_config.color;
        let visibility: Visibility = Visibility::Hidden;
    ...
    for (mut block, mut transform, movable) in &mut query {
        ...
        if manually_move_timer.0.finished() {
            if keyboard_input.pressed(KeyCode::ArrowLeft) && movable.can_left {
                ...
                spawn_drop_audio(&mut commands, &game_audios);
                reset_manually_move_timer = true;
            } else if keyboard_input.pressed(KeyCode::ArrowDown) && movable.can_down && !already_down {
                block.y -= 1;
                spawn_drop_audio(&mut commands, &game_audios);
                reset_manually_move_timer = true;
            }
...
pub fn rotate_piece(
    ...
    if keyboard_input.just_pressed(KeyCode::ArrowUp) {
        ...
        let orginal_blocks: Vec<Block> = q_piece.iter().map(|(_, block, _)| block.clone()).collect();
        // 通过矩阵变化实现旋转，可以理解为沿 y=x对称后沿y=0对称，然后平移
...
pub fn check_collision(
    ...
    // 更新Movable
    for (_, mut moveable) in &mut piece_query {
        moveable.can_left = can_left;
        moveable.can_right = can_right;
        moveable.can_down = can_down;
    }
}